...
pub struct TieredStore<N, V>
where
    N: Clone + Eq + Hash + Display + Send + Sync,
    V: Clone + Send + Sync,
{
    tiers: Vec<Arc<dyn Store<N, V>>>,
}

impl<N, V> TieredStore<N, V>
where
    ...
    V: Clone + Send + Sync + 'static,
{
    ...
    /// ])
    /// ```
    pub fn new(stores: Vec<Option<Arc<dyn Store<N, V>>>>) -> Self {
        let tiers = stores.into_iter().flatten().collect();
        TieredStore { tiers }
    }

    /// Create a tiered store from a vec of stores (no optional filtering).
    pub fn from_stores(stores: Vec<Arc<dyn Store<N, V>>>) -> Self {
        TieredStore { tiers: stores }
    }
}

#[async_trait]
impl<N, V> Store<N, V> for TieredStore<N, V>
where
    N: Clone + Eq + Hash + Display + Send + Sync + 'static,
    V: Clone + Send + Sync + 'static,
{
    ...
    async fn get(&self, namespace: N, key: &str) -> Result<Option<Entry<V>>, CacheError> {
        ...
        for (i, tier) in self.tiers.iter().enumerate() {
            let res = tier.get(namespace.clone(), key).await?;
                ...
                if i > 0 {
                    ...
                    tokio::spawn(async move {
                        for tier in lower_tiers {
                            let _ = tier
                                .set(namespace_clone.clone(), &key_clone, entry_clone.clone())
                                .await;
                        }
                    });
                }
...
mod tests {
    ...
    async fn test_tiered_get_populates_lower_tiers() {
        let l1: Arc<dyn Store<TestNamespace, String>> =
            Arc::new(HashMapStore::new(HashMapStoreConfig::default()));
        let l2: Arc<dyn Store<TestNamespace, String>> =
            Arc::new(HashMapStore::new(HashMapStoreConfig::default()));

        // Set value only in L2
        let now = now_ms();
        let entry = Entry::new("value1".to_string(), now + 60_000, now + 300_000);
        l2.set(TestNamespace::Users, "key1", entry.clone())
            .await
            .unwrap();

        // L1 should be empty
        let result = l1.get(TestNamespace::Users, "key1").await.unwrap();
        ...
        let result = tiered.get(TestNamespace::Users, "key1").await.unwrap();
